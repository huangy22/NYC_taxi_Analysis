...
if __name__ == "__main__":
    ...
    if not os.path.exists("./tmp_dropoff_{0}.csv".format(zone_name)):
        print "Generating tmp file..."
        df_dropoff = query_rides(zones, is_dropoff=True)
        df_pickup = query_rides(zones, is_dropoff=False)

        df_pickup.to_csv("tmp_pickup_{0}.csv".format(zone_name))
        df_dropoff.to_csv("tmp_dropoff_{0}.csv".format(zone_name))

    df_pickup = pd.read_csv("tmp_pickup_{0}.csv".format(zone_name))
    df_dropoff = pd.read_csv("tmp_dropoff_{0}.csv".format(zone_name))
    ...
    interval=len(df_dropoff)/(ncpu-1)
    start,end=pid*interval,(pid+1)*interval
    print start,end

    df_pickup["pickup_datetime"] = df_pickup["pickup_datetime"].apply(lambda x: pd.to_datetime(x))
    df_pickup = df_pickup.set_index(pd.DatetimeIndex(df_pickup['pickup_datetime']))
    print len(df_pickup)
    
    # delta_prev=dt.timedelta(minutes=0)
    delta_next=dt.timedelta(minutes=5)
    ...
    for index, row in df_dropoff[start:end].iterrows():

        start_time = row.dropoff_datetime  
        ...
        if DEBUG and index%10 == 0:
            print "{0} of {1}, from {2} to {3}, total {4}".format(index-start, end-start, start, end, len(df_dropoff))
    ...
    print "Success rate : ", len(df_recommend)*1.0/(end-start)*100.0, "%"
    df_recommend["region"] = zone_name

    df_recommend.to_csv(zone_name+"_recommend_next_rides_{0}.csv".format(pid))
